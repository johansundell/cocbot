...
func checkMembers(s *discordgo.Session) {
	...
	if isCocUnderUpdate {
		...
		sendMessage(s, "<@&"+everyoneId+">"+" Clash of Clans servers are up again", true)
	}
	failedTries = 0
	...
	for _, m := range members.Items {
		...
		if m.Role == "member" && m.Donations >= 1000 {
			...
			if alerted == 0 {
				...
				sendMessage(s, "<@&"+coLeaderId+">"+" Member "+m.Name+" should be upgraded", false)
		...
		if t.ErrorCode == 503 {
			...
			if failedTries > 3 {
				if !isCocUnderUpdate {
					...
					sendMessage(s, "<@&"+everyoneId+">"+" Clash of Clans servers are down", true)
				}
			}
		}
		break
